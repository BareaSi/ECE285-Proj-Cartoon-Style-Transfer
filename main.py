...
parser.add_argument('--gray_lambda', type=float, default=8, help='gray_lambda')
parser.add_argument('--adv_lambda', type=float, default=6, help='adv_lambda for generator')
...
#             print(yuv_src)
            ...
            Color_loss = 1.5*L1(yuv_src[:,0,:,:], yuv_gen[:,0,:,:])+ huber(yuv_src[:,1,:,:],yuv_gen[:,1,:,:]) + huber(yuv_src[:,2,:,:],yuv_gen[:,2,:,:])
#             print(Color_loss)
            ...
            if i % 100 == 0:
                ...
                # print("adv_loss: %.3f" % (G_adv_loss.item()))
                real = src[0].cpu().detach().numpy().transpose(1, 2, 0)
                cart = gen_cart[0].cpu().detach().numpy().transpose(1, 2, 0)
                result = np.concatenate((real, cart), axis=1)
#                 print(real.shape)
